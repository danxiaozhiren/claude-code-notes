...
| 阶段 | 持有者 | 输入 | 输出 | 不变量 |
| --- | --- | --- | --- | --- |
| 准备工具 | harness | tool schema / handler 表 | 当前可见工具池 | schema 给模型看，handler 只在 runtime 内 |
| 选择动作 | 模型 | messages + tools | `tool_use` 或最终文本 | 模型只提出意图，不直接执行 |
| 执行动作 | harness | `tool_use.name` + `tool_use.input` | handler 输出 | 执行前后可插入权限和 hooks |
| 回填观察 | harness | handler 输出 + `tool_use_id` | `tool_result` | 结果必须能对应回原始请求 |
| 继续推理 | 模型 | 更新后的 messages | 下一步动作或结束 | 外部世界变化必须进入上下文 |
