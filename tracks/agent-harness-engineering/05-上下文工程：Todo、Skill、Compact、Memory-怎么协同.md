...
| 观察点 | compact 前 | compact 后 | memory 回流 |
| --- | --- | --- | --- |
| 当前任务 | 分散在多轮 messages 和 todo 中 | 被摘要成当前目标、已完成、剩余工作 | 通常不写入长期 memory |
| 工具输出 | 可能包含长日志、文件片段、错误栈 | 只保留摘要、路径、关键结论 | 只保留稳定事实或长期偏好 |
| 用户约束 | 可能出现在最初几轮 | 必须被保留到摘要里 | 如果跨任务复用，才进入 memory |
| 技能正文 | 通过 `load_skill` 进入当前上下文 | 可被摘要成已使用规则 | skill 本体仍在外部文件 |

这个记录能验证一件事：compact 解决“当前会话继续工作”，memory 解决“未来会话还能记住稳定事实”，二者不应该互相替代。
...
本节实验对应实验手册编号：AHE-006、AHE-007、AHE-008。
