...
| hook 类型 | 失败时默认策略 | 原因 |
| --- | --- | --- |
| 权限、密钥扫描、危险命令拦截 | fail-closed | 失败意味着无法证明动作安全 |
| 审计日志、通知、指标采集 | fail-open + 记录错误 | 不应因为观测系统短暂故障阻塞低风险工作 |
| 格式化、摘要、输出压缩 | 按风险分级 | 写文件后格式化失败可以继续，但大输出治理失败可能需要阻断 |
| Stop 前一致性检查 | 有上限地重试或降级 | 避免无限续跑，也避免静默丢失收尾检查 |

这个表能把 hook 从“随便挂脚本”变成可治理的生命周期扩展点。

---

## 十五、动手实验

本节实验对应实验手册编号：AHE-005、AHE-016。
