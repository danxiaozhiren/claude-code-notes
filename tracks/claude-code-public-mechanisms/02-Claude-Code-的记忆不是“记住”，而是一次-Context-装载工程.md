...
【作者推导】这就是“记忆不是记住”的机制原因。语言模型在新 session 中不会因为上次会话而自动改变权重；它能表现得像“记得项目”，是因为稳定文件、配置、memory 索引、tool names、skill descriptions 等内容被重新放进当前 context。换句话说，跨会话记忆效果不是模型内部状态延续，而是外部资料重新装载。
...
| 已调用 skill body | 可能按预算重附加；细节见第九节 |
...
【作者推导】compact 的设计目标是在释放 context 空间的同时保留会话继续工作的语义基线。项目根 CLAUDE.md、Auto memory 这类稳定来源适合重新注入，因为它们定义了项目长期约定；对话里的临时指令、path-scoped rule、子目录 CLAUDE.md 更像局部上下文，是否恢复取决于摘要质量和后续是否再次触发相关路径。这里不要把观察结果理解成永久保证，具体行为仍应以官方文档和 `/context` 观察为准。
...
【作者推导】这种层级设计解决的是作用域问题。组织级、用户级、项目级、本地级、子目录级分别对应“所有人都要遵守”“我个人长期偏好”“这个仓库的团队约定”“我在本机的临时偏好”“某个模块的局部规则”。如果所有规则都只有一个全局入口，结果要么过长、过泛，要么局部规则污染整个项目。
...
【作者推导】`@` import 解决的是维护结构问题，不是 token 成本问题。它让你可以把一份长期说明拆成多个文件维护，但在注入时仍然等价于把被引用文件内容内联进 CLAUDE.md。想省 context，应该让 Claude 在任务相关时再读取文件，而不是用 `@` 把长文档提前展开。
...
【作者推导】Auto memory 需要审计，不是因为它的存储方式天然危险，而是因为它写入的是“模型判断未来可能有用”的经验笔记。它不是团队 PR 过的事实库，也不是组织级配置。如果一条错误经验被写入 memory，下次 session 重新装载后，Claude 可能继续受它影响。
...
【作者推导】Skills 和 CLAUDE.md 的区别，本质是常驻成本与按需成本的取舍。CLAUDE.md 适合每次会话都要知道的短规则；skill 适合偶尔才用、但一旦使用就需要完整流程和长参考资料的工作单元。把长流程塞进 CLAUDE.md，会让每次启动都支付 context 成本；做成 skill，则只有真正需要时才加载正文。
