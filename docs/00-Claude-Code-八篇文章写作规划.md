...
> 原则：所有结论必须能追溯到官方文档、官方 GitHub 仓库、可信社区实践或可复现的动手观察。  
> 校准日期：2026-05-17。
...
5. 实践验证：用 `/context`、`/mcp`、`/hooks`、`/permissions`、`claude -p` 等设计可复现练习
...
实践验证必须能被读者复现，并鼓励留下可追溯产物，至少满足一种：
...
正文中写确定性结论时，必须有官方文档或实践观察支撑。没有产物的观察不要写成“我验证过”，可以写成“你可以这样观察”。
...
**预期观察**：根据官方文档或练习假设，应该看到什么现象
**验证标准**：如何判断通过、不符合预期或文档未覆盖；建议保存什么产物
```

### 机制解释层

每篇文章不能只整理官方文档的“怎么用”，还要补一层“为什么这样设计”。遇到关键机制时，至少回答三个问题：

```text
官方事实：官方文档说它是什么、怎么工作
机制解释：这个设计背后的约束、代价和权衡是什么
工程判断：理解这个机制后，使用者应该如何做决策
```

写作时优先避免只停留在“功能描述”。例如：

- 讲 agentic loop 时，不只写 gather context / take action / verify results，还要解释语言模型不能在一次生成中主动获取外部信息，工具调用把信息获取外化成多轮 context 更新。
- 讲 Edit 时，不只写“先 Read 再 Edit”，还要解释精确字符串替换需要当前文件快照，外部修改会让快照失效。
- 讲 context 污染时，不只写“大输出会占空间”，还要解释 context window 有限、无关 token 会稀释注意力并压缩后续推理空间。
- 讲 Grep 和 LSP 时，不只写“一个搜文本一个懂代码”，还要解释词法匹配和语义分析在作用域、类型、引用关系上的差异。
- 讲 permissions 时，不只按高低风险分类，还要说明风险性质：局部修改、整体覆盖、命令副作用、外部访问、敏感信息暴露等。

机制解释属于【作者推导】，必须基于官方事实、通用架构原理或可复现观察，不写成官方结论。

### 每篇文章必须包含

- 官方主参考
- 社区或 GitHub 补充参考
- 机制解释：至少解释 2-3 个关键设计背后的约束和权衡
- 实践练习
- 易错点
- 明确区分：官方事实 / 社区实践 / 作者推导 / 实践观察
- 适合读者：个人开发者 / 团队负责人 / 平台开发者
- 实践复盘建议：与官方文档一致 / 与预期不同 / 文档未覆盖
...
- **输入**：启用不同数量的 MCP server，分别运行 `/context` 和 `/mcp`。
  **预期观察**：MCP tool definitions 是否仍然启动时全量加载，还是按官方新文档描述 deferred。
...
每个实践练习都建议按“输入 / 预期观察 / 验证标准”组织。

1. `/context`：观察 context 成本和装载内容
2. `/mcp`：观察 MCP server 与 tool 成本
3. `/hooks`：观察 hook 是否注册和触发
4. `/permissions`：检查 allow / ask / deny 来源
5. `/memory`：检查 Auto memory 写入和加载
6. `/agents`：观察 custom subagent 配置
7. `claude -p`：观察 headless 行为和 structured output
8. GitHub Actions：观察 CI 中的权限、secrets、CLAUDE.md 读取

每篇文章末尾固定增加“实践练习”和“实践复盘建议”：
